...
YEAR = 2024
OUTPUT_DIR = "conv_amsua_iasi_hms_2024"
CYCLES = ["00", "06", "12", "18"]

# Observation categories including NOAA GDAS BUFR keys
CONV_OBS_TYPES = ["adpupa", "adpsfc", "aircar", "satwnd"]
SAT_OBS_TYPES = ["1bamua", "1mtiasi", "1bhms"]  # 1bamua = AMSU-A, 1mtiasi = IASI, 1bhms = HMS

# -----------------------------------------------------------------------------
# 1. AMSU-A Channel Configuration (15 spectral channels, 23.8 to 89.0 GHz)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. HMS (Hyperspectral Microwave Sounder) Channel Configuration (12 Channels)
# -----------------------------------------------------------------------------
...
def build_unified_nc_dataset(date_str: str, cycle: str, out_file: str) -> None:
    ...
    IASI infrared radiances, and HMS microwave radiances into a standardized NetCDF dataset.
    """
    n_conv = 100000
    ...
    ds = xr.Dataset(
        ...
        attrs={
            "title": "Unified Conventional, AMSU-A, IASI, and HMS Satellite Observations for AIDA DA",
            "date": date_str,
            "cycle": f"t{cycle}z",
            "vertical_coordinate": "z (meters)",
            "conventional_variables": "p, t, td, u, v",
            "satellite_instruments": "AMSU-A (Channels 1-15), IASI (30-Channel DA Subset), HMS (Channels 1-12)",
        },
    )
...
def process_single_cycle(date_str: str, cycle: str) -> str:
    ...
    # Download raw BUFR files for conventional, AMSU-A, IASI, and HMS
...
def main():
    ...
    print("\nCompleted! Observation files generated with AMSU-A, IASI, and HMS brightness temperatures for 2024.")
