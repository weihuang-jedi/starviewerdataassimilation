...
class EdgeNodeGNNBlock(nn.Module):
    ...
    Processes both node features (channels) and edge attributes with 3-layer MLPs.
    """
    def __init__(self, channels: int, edge_dim: int = 16):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        updated_edge_attr = edge_attr_expanded + self.edge_mlp(edge_in)
...
class ScalableAIDAProcessor(nn.Module):
    ...
    def __init__(
        ...
        # Pre-allocated edge embedding for mesh distance topology (M6 grid default = 245772 edges)
        self.edge_embedding = nn.Parameter(torch.randn(245772, edge_dim) * 0.02)
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        h = self.encoder(x)
        edge_attr = self.edge_embedding[:edge_index.shape[1]]
